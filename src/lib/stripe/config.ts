...
const PLANS: Record<StripePlanName, PlanConfig> = {
  start: { id: 'start', priceId: process.env.STRIPE_PRICE_ID_START ?? '', unitsLimit: 10, displayName: 'Start' },
  scale: { id: 'scale', priceId: process.env.STRIPE_PRICE_ID_SCALE ?? '', unitsLimit: 50, displayName: 'Scale' },
  team: { id: 'team', priceId: process.env.STRIPE_PRICE_ID_TEAM ?? '', unitsLimit: 100, displayName: 'Team' },
}
