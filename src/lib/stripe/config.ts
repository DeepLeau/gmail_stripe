...
export interface PlanConfig {
  ...
  unitsLimit: number
  displayName: string
  /** Messages par mois — alias pour compatibilité UI (Pricing.tsx) */
  limit: number
  /** Prix affiché en euros — à remplir côté .env.local ou via une constante. */
  price: string
}

const PLANS: Record<StripePlanName, PlanConfig> = {
  start: { id: 'start', priceId: process.env.STRIPE_START_PRICE_ID ?? '', unitsLimit: 10, displayName: 'Start', limit: 10, price: '0 €' },
  scale: { id: 'scale', priceId: process.env.STRIPE_SCALE_PRICE_ID ?? '', unitsLimit: 50, displayName: 'Scale', limit: 50, price: '19 €' },
  team: { id: 'team', priceId: process.env.STRIPE_TEAM_PRICE_ID ?? '', unitsLimit: 100, displayName: 'Team', limit: 100, price: '49 €' },
}
