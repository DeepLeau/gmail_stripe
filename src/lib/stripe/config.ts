...
/**
 * Export direct de l'objet PLANS — consommé par les fichiers qui itèrent sur
 * les plans ou qui font un lookup par clé (ex: PLANS[slug]).
 */
export { PLANS }

/**
 * Lookup par slug (alias de getPlanById, argument en string).
 * Retourne PlanConfig ou undefined si slug invalide.
 */
export function getPlanBySlug(slug: string): PlanConfig | undefined {
  return PLANS[slug as StripePlanName]
}
