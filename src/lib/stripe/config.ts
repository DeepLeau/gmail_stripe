...
const PLANS: Record<StripePlanName, PlanConfig> = {
  start: { id: 'start', priceId: process.env.STRIPE_PRICE_START ?? '', unitsLimit: 10, displayName: 'Start' },
  scale: { id: 'scale', priceId: process.env.STRIPE_PRICE_SCALE ?? '', unitsLimit: 50, displayName: 'Scale' },
  team: { id: 'team', priceId: process.env.STRIPE_PRICE_TEAM ?? '', unitsLimit: 100, displayName: 'Team' },
}
...
// StripePlanId — export attendu par le plan consumers (§cross-file §1)
export function StripePlanId(plan: StripePlanName): string {
  return PLANS[plan].priceId
}
