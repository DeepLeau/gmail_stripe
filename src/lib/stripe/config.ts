...
const PLANS: Record<StripePlanName, PlanConfig> = {
  starter: { id: 'starter', priceId: process.env.STRIPE_PRICE_STARTER ?? '', unitsLimit: 50, displayName: 'Starter' },
  growth: { id: 'growth', priceId: process.env.STRIPE_PRICE_GROWTH ?? '', unitsLimit: 200, displayName: 'Growth' },
  pro: { id: 'pro', priceId: process.env.STRIPE_PRICE_PRO ?? '', unitsLimit: 1000, displayName: 'Pro' },
}
...
// ─── Customer management (skill stripe.md §10, cross-file §1) ───────────────
/**
 * Crée un nouveau customer Stripe ou récupère un customer existant par email.
 * Utilisé par les routes de checkout pour garantir qu'on a toujours un customer_id
 * avant de créer une Checkout Session.
 *
 * @param email - Email du user (issu de la session Supabase ou du formulaire)
 * @param userId - ID Supabase pour taguer le customer en metadata (optionnel)
 * @returns Stripe.Customer avec son id
 */
export async function createOrRetrieveCustomer(
  ...
  // Tentative de lookup par email (Stripene stocke les customers par email)
  ...
  if (existing.data.length > 0) {
    ...
    // Met à jour la metadata si userId fourni et non déjà présent
    if (userId && customer.metadata?.user_id !== userId) {
