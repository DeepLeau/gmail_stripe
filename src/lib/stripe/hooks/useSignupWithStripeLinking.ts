 ...
 *   3. Si pending_not_found : retry une fois après 2s (le webhook peut ne pas avoir encore exécuté)
...
export function useSignupWithStripeLinking(
  ...
  async function signup(email: string, password: string) {
    ...
    if (opts.pendingSessionId) {
      ...
      // La RPC retourne 'pending_not_found' quand la session n'est pas encore dans pending_checkouts.
      // On retente une fois, puis on abandonne.
      if (!result.success && result.error === 'pending_not_found') {
