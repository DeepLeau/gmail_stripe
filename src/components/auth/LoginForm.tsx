...
export function LoginForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (error) {
      setState({ status: 'error', errorMessage: 'Email ou mot de passe incorrect' })
      return
    }
    ...
    if (data.session) {
      ...
      setState({ status: 'error', errorMessage: 'Une erreur est survenue' })
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
      ...
      {state.status === 'error' && !state.fieldErrors?.email && !state.fieldErrors?.password && state.errorMessage && (
