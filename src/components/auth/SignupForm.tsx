...
type SignupFormState = {
  status: 'idle' | 'error' | 'password_mismatch'
  errorMessage?: string
  fieldErrors?: {
    email?: string
    password?: string
...
export function SignupForm({ pendingSessionId }: SignupFormProps) {
  ...
  const displayError = errorMessage ?? (state.status === 'error' ? state.errorMessage : null)
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
      ...
      {displayError && state.status !== 'password_mismatch' && (
      ...
      >
        {isLoading ? (
          <>
            ...
            <span>
              {status === 'signing_up' ? 'Création en cours...' : 'Activation de votre abonnement...'}
            </span>
          </>
        ) : (
