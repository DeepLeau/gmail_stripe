...
import { STRIPE_PLANS, UNIT_LABEL, UNIT_LABEL_PLURAL } from '@/lib/stripe/config'
...
const PLAN_DESCRIPTIONS: Record<string, string> = {
  start: 'Pour découvrir Emind sans engagement.',
  scale: 'Pour les professionnels qui vivent dans leurs emails.',
  team: 'Pour les équipes qui collaborent sur leurs boîtes mail.',
}
...
export function Pricing() {
  ...
  const plans = Object.entries(STRIPE_PLANS).map(([key, plan]) => ({
    ...
    price: PLAN_PRICES[key] ?? '0',
    period: 'mois',
    description: PLAN_DESCRIPTIONS[key] ?? '',
    features: [
      {
        text: `${plan.unitsLimit} ${plan.unitsLimit === 1 ? UNIT_LABEL : UNIT_LABEL_PLURAL} / mois`,
        included: true,
      },
      { text: 'Résumés de threads', included: true },
      { text: 'Recherche en langage naturel', included: true },
      { text: 'Multi-comptes', included: key !== 'start' },
      { text: 'Priorité de traitement', included: key === 'team' },
    ],
    cta: key === 'start' ? 'Commencer gratuitement' : `Choisir ${plan.displayName}`,
    highlighted: key === 'scale',
    badge: key === 'scale' ? 'Recommandé' : null,
  }))
  ...
  return (
    ...
    >
      <div className="max-w-4xl mx-auto">
        ...
        >
          {plans.map((plan) => {
            const badgeColors = PLAN_BADGE_COLORS[plan.id] ?? { bg: 'bg-blue-100', text: 'text-blue-700' }
            ...
            return (
              ...
              >
                ...
                <div>
                  ...
                  {/* Usage limit pill */}
                  <div className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium mb-2 ${badgeColors.bg} ${badgeColors.text}`}>
