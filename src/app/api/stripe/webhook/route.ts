 ...
 *   → on tente de retrouver via stripe_customer_id, sinon on stocke en staging
    ...
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        if (!customerId || !subscriptionId || !planMeta) {
          console.error(
            `[Stripe webhook] Incomplete checkout.session.completed (event ${event.id})`
          )
          break
        }
        ...
        if (!plan) {
          console.error(
            `[Stripe webhook] Unknown plan from price_id ${priceId} or metadata ${planMeta}`
          )
          break
        }
      ...
      case 'customer.subscription.updated': {
        ...
        const isNewPeriod =
          periodStart !== null && existing.current_period_start !== periodStart
      ...
      case 'customer.subscription.deleted': {
        ...
        if (rpcError) {
          console.error(
            `[Stripe webhook] apply_subscription_change failed on cancel:`,
            rpcError
          )
        }
        break
