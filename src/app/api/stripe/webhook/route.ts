  ...
  try {
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        // Cast via unknown pour contourner le typage strict du SDK (§6.2 skill stripe)
        const typedSub = subscription as unknown as Stripe.Subscription & {
          current_period_start: number | null
          current_period_end: number | null
        }
        const priceId = typedSub.items.data[0]?.price?.id
        ...
        const periodStart = typedSub.current_period_start != null
          ? new Date(typedSub.current_period_start * 1000).toISOString()
          : null
        const periodEnd = typedSub.current_period_end != null
          ? new Date(typedSub.current_period_end * 1000).toISOString()
          : null
        ...
        const { error: rpcError } = await supabase.rpc('apply_subscription_change', {
          ...
          p_subscription_status: typedSub.status,
      ...
      case 'customer.subscription.updated': {
        ...
        // Cast via unknown pour les champs période (§6.2 skill stripe)
