...
export async function POST(request: NextRequest) {
  ...
  // Cast as any requis dans le webhook pour éviter les erreurs TS "not assignable to never"
  // sur les écritures RPC (service_role bypass RLS, signature Stripe déjà vérifiée)
  ...
  try {
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        // Cast narrow sur les champs nécessaires plutôt que sur l'objet entier
      ...
      case 'customer.subscription.updated': {
        ...
        // Cast narrow sur les champs utilisés dans ce handler
        const subscription = rawSub as unknown as {
