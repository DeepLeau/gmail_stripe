    ...
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        const { error: rpcError } = await supabase.rpc('apply_subscription_change', {
          p_user_id: (existing as { user_id: string } | null)?.user_id ?? null,
          p_plan: plan,
      ...
      case 'customer.subscription.updated': {
        ...
        const existingUser = existing as { user_id: string; current_period_start: string | null }
        const isNewPeriod = periodStart !== null
          && existingUser.current_period_start !== periodStart
      ...
      case 'customer.subscription.deleted': {
        ...
        if (!(existing as { user_id: string } | null)?.user_id) break

        const { error: rpcError } = await supabase.rpc('apply_subscription_change', {
          p_user_id: (existing as { user_id: string }).user_id,
          p_plan: null,
          p_units_limit: 0,
          p_stripe_customer_id: customerId,
