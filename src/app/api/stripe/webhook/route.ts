 ...
 * - customer.subscription.updated : renouvellements (l'user existe forcément)
  ...
  try {
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        // stripe.subscriptions.retrieve() retourne un objet Subscription direct (pas un Response wrapper).
        const subRaw = await stripe.subscriptions.retrieve(subscriptionId)
        const subscription = subRaw as unknown as Stripe.Subscription & {
