...
export async function POST(request: NextRequest) {
  ...
  // Cast as any pour éviter les `never` sur writes webhook
  // (service_role bypass RLS, signature vérifiée en amont)
  ...
  try {
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        // stripe.subscriptions.retrieve() returns Response<Subscription>
        ...
        const { error: rpcError } = await sb.rpc('apply_subscription_change', {
      ...
      case 'customer.subscription.updated': {
        // Cast via unknown to satisfy TS2352 — Response<Subscription> → Subscription cast
