    ...
    switch (event.type) {
      ...
      case 'customer.subscription.updated': {
        ...
        if (!existing?.user_id) {
          ...
          const periodStart = subRaw.current_period_start != null
            ? new Date(subRaw.current_period_start * 1000).toISOString()
            : null
          const periodEnd = subRaw.current_period_end != null
            ? new Date(subRaw.current_period_end * 1000).toISOString()
            : null
          await supabase
            .from('pending_checkouts')
            .update({
              subscription_status: subRaw.status,
        ...
        const priceId = subRaw.items.data[0]?.price?.id
        ...
        const { error: rpcError } = await supabase.rpc('apply_subscription_change', {
          ...
          p_units_limit: getPlanLimit(plan),
          p_stripe_customer_id: customerId,
          p_stripe_subscription_id: subRaw.id,
          p_stripe_session_id: null,
          p_subscription_status: subRaw.status,
          p_current_period_start: periodStart,
          p_current_period_end: periodEnd,
