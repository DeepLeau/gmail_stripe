...
export async function POST(request: NextRequest) {
  ...
  // Cast `as any` sur le client service_role pour éviter les erreurs TS "not assignable to never"
  // sur les INSERT/UPDATE/UPSERT — acceptable ici car la signature Stripe est déjà vérifiée
  // et service_role bypass RLS de toute façon (§5.2 du skill Stripe).
  ...
  try {
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        // On passe par `unknown` d'abord pour éviter TS2352 (§6 du skill Stripe).
        const rawSubscription = await stripe.subscriptions.retrieve(subscriptionId)
        const subscription = rawSubscription as unknown as SubscriptionWithPeriod
