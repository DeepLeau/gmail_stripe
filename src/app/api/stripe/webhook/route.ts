  ...
  try {
    switch (event.type) {
      ...
      case 'customer.subscription.updated': {
        const rawSub = event.data.object as unknown as Stripe.Subscription & {
          current_period_start: number | null
          current_period_end: number | null
        }
        const customerId = typeof rawSub.customer === 'string' ? rawSub.customer : null
        if (!customerId) break
        ...
        if (!existing?.user_id) {
          ...
          const periodStart = rawSub.current_period_start != null
            ? new Date(rawSub.current_period_start * 1000).toISOString()
            : null
          const periodEnd = rawSub.current_period_end != null
        ...
        const { error: rpcError } = await supabase.rpc('apply_subscription_change', {
          ...
          p_units_limit: getPlanLimit(plan),
          p_stripe_customer_id: customerId,
          p_stripe_subscription_id: rawSub.id,
          p_stripe_session_id: null, // pas applicable pour un update
          p_subscription_status: rawSub.status,
          p_current_period_start: periodStart,
          p_current_period_end: periodEnd,
